...
const CartItem = ({ item }: { item: ICartItem }) => {
  ...
  const handleRemoveItem = async () => {
    ...
    if (res.status === "success") {
      notify("تم حذف المنتج من السلة بنجاح", "success");
      router.refresh()
    } else {
      notify("فشل في حذف المنتج من السلة", "error");
    }
  }
  ...
  return (
    ...
    <div className="w-full my-4 px-4 py-4 flex flex-col items-center bg-white rounded-lg shadow-md sm:flex-row sm:items-start">
      ...
      <div className="w-full mt-4 sm:mt-0 sm:ml-4  ">
        <div className="flex justify-between items-center">
          <span className="text-gray-500 text-sm">{item.product.category.name}</span>
          <div onClick={handleRemoveItem} className="flex items-center text-white p-1 rounded-md bg-red-600 cursor-pointer">
            <Trash2Icon size={20} />
          </div>
        </div>
